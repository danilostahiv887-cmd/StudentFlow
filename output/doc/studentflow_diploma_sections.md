...
1. Предметна область StudentFlow, постановка задачі та аналіз рішень для студентського залучення  
1.1. Позааудиторний маршрут як об’єкт автоматизації  
1.2. Порівняння campus engagement та co-curricular платформ  
1.3. Обґрунтування підходу до реалізації StudentFlow  

2. Проєктування маршруту, алгоритмів та інформаційної моделі  
2.1. Архітектура взаємодії ролей і сервісів  
2.2. Алгоритми проходження активності та обробки доказів  
2.3. Моделі функціонування основних контурів системи  
2.4. Дані, зв’язки та схема Supabase Postgres  

3. Програмна реалізація вебзастосунку  
3.1. Об’єктна модель домену  
3.2. Бібліотеки та зовнішні сервіси  
3.3. Реалізація серверних і клієнтських модулів  
3.4. Користувацькі інтерфейси за ролями  
3.5. Відхилені технічні варіанти  
3.6. Нестандартні ситуації під час розробки  

4. Перевірка роботи та умови експлуатації  
4.1. Практичні інструкції для ролей системи  
4.2. Мінімальне програмно-апаратне середовище  
4.3. Перевірка функціональних сценаріїв  
4.4. Підсумкова оцінка результатів  
...
- початкове наповнення виконується тільки для порожньої бази, щоб production-дані не перезаписувалися при запуску.

Нефункціональні вимоги стосуються практичної експлуатації. Інтерфейс повинен бути адаптивним, модальні форми мають коректно працювати на малих екранах, пошук і пагінація потрібні у великих списках, production-збірка не повинна містити dev-індикаторів, а при тимчасово paused Supabase користувач має бачити зрозумілий екран очікування тільки у випадку реальної недоступності бази.
## 1.2. Порівняння campus engagement та co-curricular платформ

Для порівняння обрано не загальні LMS і не сервіси керування уроками, а рішення з близької сфери: студентське залучення, co-curricular transcript, campus life, події, організації, цифровий запис позааудиторного досвіду та підтвердження участі. Такий підбір аналогів точніше відповідає темі StudentFlow, оскільки система не замінює навчальний курс, а організовує активності поза аудиторією.

**Suitable** є одним із найближчих аналогів за ідеєю co-curricular transcript. Платформа описує позааудиторний запис як динамічне цифрове представлення досвіду студента, де участь, компетентності, досягнення та milestones формують цілісну історію розвитку. Для StudentFlow важливою стала саме ця логіка: студенту потрібен не список відвіданих подій, а зрозумілий маршрут, який можна показати як портфоліо. Відмінність StudentFlow полягає у локальній моделі коледжу: спеціальності, групи, викладачі, категорії та правила відзнак налаштовуються без великої комерційної екосистеми.

**Modern Campus Involve** орієнтований на студентське залучення, організації, події, co-curricular learning і відстеження розвитку навичок. Це рішення демонструє, що позааудиторні події мають бути пов’язані не тільки з календарем, а й із retention, участю в організаціях та розвитком компетентностей. У StudentFlow цю ідею звужено до дипломного вебзастосунку: замість масштабної платформи для кампусу реалізовано компактні ролі студента, викладача й адміністратора, а контроль результату перенесено у доказ і фідбек.

**CampusGroups by Ready Education** поєднує campus life, студентські організації, події, комунікацію, guided pathways і co-curricular transcript. Для StudentFlow цей аналог є важливим через підхід до маршрутів: студент рухається не хаотично, а через набір напрямів, очікувань і зафіксованих результатів. Водночас CampusGroups ширший за StudentFlow: він охоплює мобільний кампусний досвід, комунікації, організації, інтеграції та інституційні процеси. У власній системі залишено ядро, потрібне для коледжу: активність, маршрут, доказ, відзнака, група та адміністративне керування.

**Anthology Engage** зосереджується на student organizations, events, involvement data, API та інтеграціях. Його цінність для аналізу полягає у тому, що позааудиторна активність трактується як джерело даних для підтримки студентського успіху, а не як другорядний журнал подій. StudentFlow не копіює інтеграційну модель Anthology, але використовує схожий принцип: кожна дія студента залишає структурований запис, який може переглянути викладач або адміністратор.

**Pathify Communities** розвиває ідею campus experience hub: групи, події, чати, co-curricular tracking і цифрова спільнота об’єднуються в єдиному середовищі. Для StudentFlow важливою є не соціальна мережа кампусу, а сам підхід “єдиного входу” до студентського досвіду. Тому в застосунку зроблено окремий навігатор активностей і кабінети ролей, але не реалізовано чат, стрічку повідомлень або повноцінний campus portal.

**GivePulse** розглянуто як суміжний, але не базовий аналог. Його сильна сторона — service-learning, volunteer management, tracking, reflections, digital badges і transcripts. Для StudentFlow корисними є механіки підтвердження результату та рефлексії, однак предметна область ширша за волонтерство: активності охоплюють дослідження, прототипування, медіа, баланс, лідерство, кар’єру та комунікацію.
...
| --- | --- | --- | --- |
| Suitable | Co-curricular transcript, компетентності, портфоліо | Маршрут, бали, відзнаки, підсумковий профіль | Потрібна локальна структура груп, спеціальностей і викладачів |
| Modern Campus Involve | Події, організації, co-curricular learning | Категорії активностей, зв’язок із навичками, роль ментора | Готова платформа надлишкова для коледжного дипломного проєкту |
| CampusGroups | Guided pathways, student life, CCT | Напрями розвитку, кроки маршруту, портфоліо | Не потрібні широкі campus-комунікації та платіжні/організаційні модулі |
| Anthology Engage | Involvement data, організації, API | Структурований запис участі й адміністративний огляд | Орієнтація на інституційні інтеграції, яких немає в локальному сценарії |
| Pathify Communities | Єдиний digital hub для campus life | Навігатор і рольові кабінети як єдиний вхід | Соціальні функції й чат не входять у мету StudentFlow |
| GivePulse | Service-learning, reflections, badges | Докази участі, фідбек, значення рефлексії | Фокус сервісу — громадська й волонтерська активність, а не весь позааудиторний маршрут |
...
StudentFlow реалізовано як full-stack застосунок на Next.js App Router. Такий підхід обрано через потребу тримати в одному проєкті публічні сторінки, role-based dashboard, server actions, API-маршрути, доступ до Supabase, роботу з cookies і клієнтські інтерактивні форми. Для дипломного проєкту це практичніше, ніж розділяти систему на окремий backend і frontend, оскільки вся логіка маршруту студента зберігається поруч із відповідними сторінками.

Технологічний набір підібрано не за принципом демонстрації популярних бібліотек, а за задачами системи:

- **Next.js 15** використано для маршрутизації, server-rendered сторінок, API endpoints і production build;
...
- **bcryptjs** застосовано для хешування паролів seed-акаунтів і створених користувачів;
- **Zod** відповідає за серверну валідацію форм, де помилка має бути відловлена до запису в базу;
- **Render** обрано як цільове середовище запуску production-версії.

Алгоритмічна модель системи будується навколо статусів. Запис `applications` показує, що студент додав активність до маршруту або вже пройшов її. Запис `reports` містить доказ і стан перевірки. Після прийняття доказу студент отримує бали, а механізм відзнак може видати badge за кількість балів, кількість активностей або конкретний напрям. Така модель дає прозорий ланцюжок: активність → маршрут → доказ → фідбек → портфоліо.

Окреме рішення прийнято для роботи з медіафайлами. Зображення не повинно завантажуватися у хмару в момент простого вибору файлу, бо користувач може закрити сторінку або не підтвердити форму. Тому StudentFlow завантажує файл тільки під час submit. Якщо запис у Supabase не створився або редагування не завершилося, серверна логіка видаляє завантажений файл з ImageKit. Це прибирає проблему “сирітських” медіа.

Ще один практичний елемент — робота з paused Supabase на безкоштовному тарифі. Застосунок не показує повідомлення про запуск бази при кожному переході. Екран очікування вмикається тільки тоді, коли сервер справді отримав помилку недоступності проєкту або статус, який потребує відновлення. Після цього модуль пробудження періодично перевіряє готовність бази.
# 2. ПРОЄКТУВАННЯ МАРШРУТУ, АЛГОРИТМІВ ТА ІНФОРМАЦІЙНОЇ МОДЕЛІ
## 2.1. Архітектура взаємодії ролей і сервісів
...
9. Suitable. The Co-Curricular Transcript. URL: https://www.suitable.co/products/co-curricular-transcripts
10. Suitable. The Co-Curricular Transcript: What Is It? URL: https://www.suitable.co/knowledge-center/blog/the-co-curricular-transcript-what-is-it
...
12. Ready Education. CampusGroups: Your Student Engagement Solution. URL: https://www.readyeducation.com/campusgroups/
13. Anthology Engage / Encoura. Student Engagement Platform. URL: https://encoura.org/anthology/enrollment-and-retention/engage
14. Pathify Communities. Campus Engagement Hub. URL: https://pathify.com/engagement-hub/communities/
15. GivePulse. Higher Education. URL: https://learn.givepulse.com/higher-education
# ДОДАТКИ
