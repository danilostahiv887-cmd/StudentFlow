...
export function activityView(
  ...
  activity: Activity,
  counts = approvedCounts(database),
): ActivityView {
...
export function buildActivityPage(database: DatabaseSnapshot, filters: ActivityFilters = {}) {
  ...
  if (filters.available === 'yes')
    items = items.filter((item) => item.maxParticipants - (counts.get(item.id) ?? 0) > 0);
  ...
  return {
    ...page,
    items: page.items.map((item) => activityView(database, item, counts)),
...
export function enrichApplication(database: DatabaseSnapshot, application: Application) {
  const activity = activityView(
    database,
    database.activities.find((item) => item.id === application.activityId)!,
  );
...
export async function studentData(studentId: string) {
  ...
  const applications = database.applications
    ...
    .map((item) => enrichApplication(database, item));
  const reports = database.reports.filter((item) => item.studentId === studentId);
  const unlocked = database.studentBadges
    .filter((item) => item.studentId === studentId)
    .map((item) => ({
      ...item,
      badge: database.badges.find((badge) => badge.id === item.badgeId)!,
    }));
...
export async function teacherQueue(teacherId: string) {
  const database = await readDatabase();
  const activityIds = database.activities
    .filter((item) => item.teacherId === teacherId)
    .map((item) => item.id);
  const applications = database.applications
    .filter((item) => activityIds.includes(item.activityId))
    .map((item) => enrichApplication(database, item));
  const reports = database.reports
    ...
    .map((item) => ({
      ...
      application: enrichApplication(
        database,
        database.applications.find((app) => app.id === item.applicationId)!,
      ),
      student: database.profiles.find((profile) => profile.id === item.studentId)!,
    }));
  return {
    database,
    activities: database.activities
      .filter((item) => activityIds.includes(item.id))
      .map((item) => activityView(database, item)),
    applications,
    reports,
  };
}
...
export async function adminData() {
  ...
  return {
    database,
    activities: database.activities.map((item) => activityView(database, item)),
    students: database.profiles.filter((item) => item.role === 'student'),
    teachers: database.profiles.filter((item) => item.role === 'teacher'),
  };
}

export async function authenticate(email: string, password: string): Promise<Profile | undefined> {
  const profile = (await readDatabase()).profiles.find(
    (item) =>
      item.email.toLocaleLowerCase() === email.toLocaleLowerCase() && item.status === 'active',
  );
