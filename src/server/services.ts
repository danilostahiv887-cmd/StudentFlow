...
export const applicationSchema = z.object({
  ...
  motivation: z.string().trim().min(10, 'Опишіть, для чого додаєте цей крок.').max(1000),
});
export const reportSchema = z.object({
  applicationId: z.string(),
  reflection: z.string().trim().min(20, 'Доказ має містити щонайменше 20 символів.'),
  hoursSpent: z.coerce.number().positive('Вкажіть додатну кількість годин.'),
  skillsGained: z.string().trim().min(3, 'Вкажіть компетентності.'),
...
async function deleteUnusedMedia(database: Database, kind: MediaAsset['kind'], imageKey: number) {
  ...
  const assets = database.mediaAssets.filter(
    (item) => item.kind === kind && item.imageKey === imageKey,
  );
  for (const asset of assets) await deleteImageKitFile(asset.fileId);
  database.mediaAssets = database.mediaAssets.filter(
...
export async function cancelApplication(actor: Profile, applicationId: string, reason: string) {
  ...
  if (!application || application.studentId !== actor.id)
    throw new DomainError('Крок маршруту не знайдено.');
  if (!['submitted', 'under_review', 'approved'].includes(application.status))
    throw new DomainError('Цей крок уже завершено або прибрано.');
  if (application.status === 'approved' && reason.trim().length < 5)
    throw new DomainError('Коротко вкажіть причину.');
...
export async function submitReport(actor: Profile, input: unknown) {
  ...
  const application = database.applications.find(
    (item) => item.id === values.applicationId && item.studentId === actor.id,
  );
  ...
  if (existing && ['approved', 'rejected'].includes(existing.status))
    throw new DomainError('Цей доказ уже розглянуто.');
...
export async function reviewApplication(
  ...
  const activity = database.activities.find((item) => item.id === application.activityId)!;
  if (actor.role !== 'admin' && !(actor.role === 'teacher' && ownedActivity(actor.id, activity)))
    throw new DomainError('Немає доступу до цього маршруту.');
  if (status === 'rejected' && comment.trim().length < 5)
    throw new DomainError('Для відкладення вкажіть причину.');
  if (status === 'approved') {
    const approved = database.applications.filter(
      (item) => item.activityId === activity.id && ['approved', 'attended'].includes(item.status),
    ).length;
    if (approved >= activity.maxParticipants) throw new DomainError('Немає вільних місць.');
  }
  application.status = status;
  application.teacherComment = comment.trim();
  application.rejectionReason = status === 'rejected' ? comment.trim() : undefined;
  application.updatedAt = now();
  await writeDatabase(database);
}
...
export async function reviewReport(
  ...
  if ((status === 'rejected' || status === 'needs_changes') && feedback.trim().length < 5)
    throw new DomainError('Додайте зрозумілий коментар для студента.');
...
function unlockBadges(database: Awaited<ReturnType<typeof readDatabase>>, student: Profile) {
  const attended = database.applications.filter(
    (item) => item.studentId === student.id && item.status === 'attended',
  );
    ...
    const condition =
      badge.conditionType === 'points'
        ...
        : badge.conditionType === 'activities'
          ...
          : attended.some(
              (application) =>
                database.activities.find((activity) => activity.id === application.activityId)
                  ?.categoryId === badge.categoryId,
            );
    if (
      condition &&
      !database.studentBadges.some(
        (item) => item.studentId === student.id && item.badgeId === badge.id,
      )
    )
...
export async function updateProfile(
  ...
  const database = await readDatabase();
  const profile = database.profiles.find((item) => item.id === actor.id)!;
  ...
  if (actor.role === 'student') {
    if (!input.groupId || !input.specialityId)
      throw new DomainError('Оберіть групу та спеціальність.');
    profile.groupId = input.groupId;
    profile.specialityId = input.specialityId;
  }
  await writeDatabase(database);
}
...
export async function toggleUser(actor: Profile, profileId: string) {
  if (actor.role !== 'admin')
    throw new DomainError('Лише адміністратор може змінювати статус користувачів.');
  const database = await readDatabase();
  const profile = database.profiles.find((item) => item.id === profileId);
  if (!profile || profile.role === 'admin')
    throw new DomainError('Обліковий запис адміністратора не можна деактивувати.');
...
export async function registerStudent(input: {
  ...
  if (!z.string().email().safeParse(input.email).success)
    throw new DomainError('Вкажіть коректну електронну пошту.');
  if (input.password.length < 6) throw new DomainError('Пароль має містити щонайменше 6 символів.');
  if (
    database.profiles.some(
      (item) => item.email.toLocaleLowerCase() === input.email.trim().toLocaleLowerCase(),
    )
  )
    throw new DomainError('Користувач із такою поштою вже існує.');
...
function adminOnly(actor: Profile) {
  if (actor.role !== 'admin') throw new DomainError('Ця дія доступна лише адміністратору.');
}
export async function createTeacher(
  ...
  if (
    ...
    throw new DomainError('Заповніть ім’я, коректну пошту та пароль від 6 символів.');
  if (
    database.profiles.some(
      (item) => item.email.toLocaleLowerCase() === input.email.toLocaleLowerCase(),
    )
  )
    throw new DomainError('Користувач із такою поштою вже існує.');
  ...
  database.profiles.push(teacher);
  await writeDatabase(database);
}
export async function createActivity(
  ...
  if (
    ...
    throw new DomainError('Перевірте назву, довідники, дати, місткість і кількість балів.');
  const slugBase =
    input.title
      .trim()
  ...
  database.activities.push({
    ...
    difficulty: 'beginner',
    status: 'published',
    requirements: 'Оберіть очікуваний доказ перед стартом.',
    resultDescription: 'Доказ у портфоліо та оновлена карта компетентностей.',
    skills: ['Самоменеджмент', 'Командна робота'],
  });
  await writeDatabase(database);
}
export async function createReference(
  actor: Profile,
  kind: 'groups' | 'specialities' | 'clubs' | 'categories' | 'badges',
  input: {
    name: string;
    code?: string;
    teacherId?: string;
  ...
  adminOnly(actor);
  const database = await readDatabase();
  if (!input.name.trim()) throw new DomainError('Вкажіть назву.');
  const id = randomUUID();
  const slug = input.name
    .trim()
    .toLocaleLowerCase('uk-UA')
    .replace(/[^a-zа-яіїєґ0-9]+/gi, '-');
  if (kind === 'specialities')
    database.specialities.push({
      id,
      code: input.code?.trim().toUpperCase() || `SP-${database.specialities.length + 1}`,
      name: input.name.trim(),
      description: input.description?.trim() || '',
    });
  if (kind === 'groups') {
    const specialityId = database.specialities[0]?.id;
    if (!specialityId) throw new DomainError('Спершу створіть спеціальність.');
    const startYear = new Date().getFullYear();
    database.groups.push({
      id,
      name: `${input.name.trim()} (${startYear}-${startYear + 4})`,
      specialityId,
      startYear,
      endYear: startYear + 4,
      isActive: true,
  ...
  if (kind === 'clubs') {
    const teacherId =
      input.teacherId || database.profiles.find((item) => item.role === 'teacher')?.id;
    if (!teacherId) throw new DomainError('Спершу створіть викладача.');
    const fallbackImageKey = (database.clubs.length % 8) + 1;
    database.clubs.push({
      id,
      name: input.name.trim(),
      slug,
  ...
  if (kind === 'categories') {
    const fallbackImageKey = (database.categories.length % 8) + 1;
  ...
  if (kind === 'badges') {
    const fallbackImageKey = (database.badges.length % 8) + 1;
    database.badges.push({
      id,
      title: input.name.trim(),
      description: input.description?.trim() || 'Нова відзнака StudentFlow.',
      imageKey: attachMedia(database, 'badge', input, input.name.trim()) ?? fallbackImageKey,
      color: input.color || input.code || 'aqua',
...
export async function grantStudentBadge(
  actor: Profile,
  input: { studentId: string; badgeId: string },
) {
  adminOnly(actor);
  const database = await readDatabase();
  const student = database.profiles.find(
    (item) => item.id === input.studentId && item.role === 'student',
  );
...
export async function updateAdminEntity(
  actor: Profile,
  input: { entity: string; id: string; values: Record<string, string> },
) {
  adminOnly(actor);
  const database = await readDatabase();
  ...
  if (input.entity === 'profile') {
    ...
    if (!profile || profile.role === 'admin')
      throw new DomainError('Профіль не знайдено або недоступний для редагування.');
    ...
    if (values.email?.trim() && z.string().email().safeParse(values.email).success)
      profile.email = values.email.trim().toLocaleLowerCase();
  ...
  if (input.entity === 'activity') {
    ...
    if (values.resultDescription?.trim())
      activity.resultDescription = values.resultDescription.trim();
    if (values.categoryId && database.categories.some((item) => item.id === values.categoryId))
  ...
  if (input.entity === 'group') {
    ...
    const baseName = (values.name?.trim() || group.name)
  ...
  if (input.entity === 'club') {
    ...
    if (
      values.teacherId &&
      database.profiles.some(
        (profile) => profile.id === values.teacherId && profile.role === 'teacher',
      )
    )
  ...
  if (input.entity === 'badge') {
    ...
    await applyMediaChange(database, 'badge', item.imageKey, 1, values, item.title, (value) => {
      item.imageKey = value;
    });
  }
  await writeDatabase(database);
...
export async function deleteAdminEntity(actor: Profile, input: { entity: string; id: string }) {
  ...
  if (input.entity === 'profile') {
    ...
    if (profile.role === 'student') {
      ...
      database.studentBadges = database.studentBadges.filter(
  ...
  if (input.entity === 'speciality') {
    database.profiles
      .filter((item) => item.specialityId === input.id)
      .forEach((item) => {
        item.specialityId = undefined;
      });
    database.groups
      .filter((item) => item.specialityId === input.id)
      .forEach((item) => deleteGroup(item.id));
