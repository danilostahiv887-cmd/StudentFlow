...
export const failure = (error: unknown) => {
  ...
  if (isDatabaseStartingError(error)) return NextResponse.json({ success: false, formError: 'База даних запускається. Повторіть дію за кілька секунд.', databaseState: 'starting' }, { status: 503 });
