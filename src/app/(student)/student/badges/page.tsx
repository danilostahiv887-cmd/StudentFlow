...
export default async function BadgesPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const imageUrl = (imageKey: number) =>
    database.mediaAssets.find((item) => item.kind === 'badge' && item.imageKey === imageKey)?.url;
  let badges = database.badges.filter(
    (badge) =>
      !needle || `${badge.title} ${badge.description}`.toLocaleLowerCase('uk-UA').includes(needle),
  );
  if (query.state === 'unlocked')
    badges = badges.filter((badge) => unlocked.some((item) => item.badgeId === badge.id));
  if (query.state === 'locked')
    badges = badges.filter((badge) => !unlocked.some((item) => item.badgeId === badge.id));
  ...
  return (
    <div className="page">
      <div className="page-intro">
        ...
        <p>Кожна відзнака відображає підтверджений крок маршруту.</p>
      </div>
      <div className="workspace">
        <WorkspaceNav role="student" active="/student/badges" />
        <div className="workspace-main">
          <section className="surface">
            <ListControls
              ...
              search={query.search}
              pageSize={pageSize}
              placeholder="Назва або опис відзнаки"
            >
              <AppSelect name="state" defaultValue={query.state ?? ''}>
                <option value="">Усі відзнаки</option>
                <option value="unlocked">Відкриті</option>
                <option value="locked">Ще закриті</option>
              </AppSelect>
            </ListControls>
            {result.items.length ? (
              <div className="badge-grid">
                {result.items.map((badge) => {
                  const item = unlocked.find((value) => value.badgeId === badge.id);
                  return (
                    <BadgeCard
                      badge={badge}
                      unlockedAt={item?.unlockedAt}
                      progress={item ? 100 : 35}
              ...
              <EmptyState title="Відзнак не знайдено" body="Змініть пошук або фільтр колекції." />
            )}
            <Pagination
              page={result.page}
