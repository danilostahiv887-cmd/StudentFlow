...
export default async function BadgesPage() {
  ...
  return (
    <div className="page">
      ...
      <div className="workspace"><WorkspaceNav role="student" active="/student/badges" /><div className="workspace-main"><section className="surface"><div className="badge-grid">{database.badges.map((badge) => { const item = unlocked.find((value) => value.badgeId === badge.id); return <BadgeCard badge={badge} unlockedAt={item?.unlockedAt} progress={item ? 100 : 35} imageUrl={imageUrl(badge.imageKey)} key={badge.id} />; })}</div></section></div></div>
    </div>
  );
}
