...
export default async function AdminStudentsPage({
  ...
  const filtered = students.filter(
    (student) =>
      !search || `${student.fullName} ${student.email}`.toLocaleLowerCase('uk-UA').includes(search),
  );
  ...
  return (
    <div className="page">
      <div className="page-intro">
        ...
        <p>Керування студентськими профілями та перегляд маршрутів.</p>
      </div>
            ...
            {result.items.length ? (
              <div className="row-list">
                {result.items.map((student) => {
                  const group =
                    database.groups.find((item) => item.id === student.groupId)?.name ||
                    'Не вибрано';
                  return (
                    <article className="data-row" key={student.id}>
                      ...
                      <small>{student.pointsTotal} балів</small>
                      <div className="data-row-actions">
                        ...
                        <ToggleUserButton profileId={student.id} status={student.status} />
                        <AdminCrudActions
                          entity="profile"
                          id={student.id}
                          title={student.fullName}
                          fields={[
                            { name: 'fullName', label: 'Повне ім’я', value: student.fullName },
                            { name: 'email', label: 'Пошта', type: 'email', value: student.email },
              ...
              <EmptyState title="Нічого не знайдено" body="Змініть пошуковий запит." />
            )}
            <Pagination
              page={result.page}
