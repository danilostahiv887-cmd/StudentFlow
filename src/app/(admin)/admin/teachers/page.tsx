...
export default async function AdminTeachersPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const filtered = teachers.filter((teacher) => !needle || `${teacher.fullName} ${teacher.email}`.toLocaleLowerCase('uk-UA').includes(needle));
  ...
  return (
    <div className="page">
      <div className="page-intro">
        <div><p className="eyebrow">Адміністративний контур</p><h1>Ментори</h1></div>
      ...
      <div className="workspace">
        ...
        <div className="workspace-main">
          <section className="surface">
            <ListControls pathname="/admin/teachers" search={query.search} pageSize={pageSize} placeholder="Ім’я або пошта" />
            {result.items.length ? (
              <div className="row-list">
                {result.items.map((teacher) => (
                  <article className="data-row" key={teacher.id}>
                    <div><b>{teacher.fullName}</b><small>{teacher.email} · {activities.filter((activity) => activity.teacherId === teacher.id).length} треків</small></div>
                    <StatusBadge status={teacher.status} />
                    <small>Роль: ментор</small>
                    <div className="data-row-actions">
                      <ToggleUserButton profileId={teacher.id} status={teacher.status} />
                      <AdminCrudActions entity="profile" id={teacher.id} title={teacher.fullName} fields={[
            ...
            <Pagination page={result.page} pageCount={result.pageCount} total={result.total} pathname="/admin/teachers" query={listQuery} />
          </section>
        </div>
      </div>
    </div>
