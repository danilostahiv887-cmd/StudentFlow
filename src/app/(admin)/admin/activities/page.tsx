...
export default async function AdminActivitiesPage({
  ...
  let filtered = activities.filter(
    (activity) =>
      !needle ||
      `${activity.title} ${activity.shortDescription} ${activity.category.name} ${activity.club.name} ${activity.teacher.fullName}`
        .toLocaleLowerCase('uk-UA')
        .includes(needle),
  );
  if (query.category)
    filtered = filtered.filter((activity) => activity.categoryId === query.category);
                      ...
                      fields={[
                        {
                          ...
                          alt: activity.title,
                          help: 'Замініть або видаліть зображення можливості.',
                        },
