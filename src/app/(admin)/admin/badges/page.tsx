            ...
            {result.items.length ? (
              <div className="badge-grid">
                {result.items.map((badge) => {
                  const media = db.mediaAssets.find(
                    (item) => item.kind === 'badge' && item.imageKey === badge.imageKey,
                  );
                  return (
                    <div key={badge.id}>
                      ...
                      <AdminCrudActions
                        ...
                        fields={[
                          {
                            name: 'image',
                            label: 'Зображення відзнаки',
                            type: 'image',
                            value: media?.url ?? '',
                            alt: badge.title,
                            help: 'Зображення буде показане на картці відзнаки.',
                          },
              ...
              <EmptyState title="Відзнак не знайдено" body="Змініть пошук або стан." />
            )}
            <Pagination
              page={result.page}
