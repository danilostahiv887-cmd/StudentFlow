...
export default async function AdminReportsPage({
  ...
  if (needle)
    reports = reports.filter((report) => {
      ...
      return `${student?.fullName ?? ''} ${student?.email ?? ''} ${activity?.title ?? ''} ${report.reflection} ${report.skillsGained} ${report.teacherFeedback ?? ''}`
  ...
  return (
    <div className="page">
      <div className="page-intro">
        ...
        <p>Перегляд артефактів, рефлексій і менторського фідбеку.</p>
      </div>
      <div className="workspace">
        <WorkspaceNav role="admin" active="/admin/reports" />
          ...
          {result.items.length ? (
            <div className="row-list">
              {result.items.map((report) => {
                const student = db.profiles.find((x) => x.id === report.studentId);
                const activity = db.activities.find((x) => x.id === report.activityId);
                return (
                  <article className="data-row route-data-row" key={report.id}>
                    <div>
            ...
            <EmptyState title="Доказів не знайдено" body="Змініть пошук або стан доказу." />
          )}
          <Pagination
            page={result.page}
