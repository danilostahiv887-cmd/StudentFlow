...
export function Pagination({ page, pageCount, total, pathname, query }: { page: number; pageCount: number; total: number; pathname: string; query: Record<string, string | undefined> }) {
  const href = (target: number) => `${pathname}?${new URLSearchParams(Object.entries({ ...query, page: String(target) }).filter(([, value]) => Boolean(value)) as [string, string][]).toString()}`;
  return <nav className="pagination" aria-label="Пагінація"><span>{total} записів · сторінка {page} з {pageCount}</span><div><Link aria-disabled={page <= 1} className="icon-button" href={href(Math.max(1, page - 1))}><ChevronLeft size={16} /></Link><Link aria-disabled={page >= pageCount} className="icon-button" href={href(Math.min(pageCount, page + 1))}><ChevronRight size={16} /></Link></div></nav>;
}
...
export function ActivityCard({ activity }: { activity: ActivityView }) { return <article className="activity-card opportunity-card"><Link href={`/activities/${activity.slug}`} className="activity-cover opportunity-map"><ImageKitImage imageKey={activity.imageKey} url={activity.imageUrl} alt={activity.imageAlt ?? activity.title} priority={activity.imageKey <= 3} /><span className={`status-rail rail-${activity.category.color}`} /></Link><div className="activity-body"><div className="card-top"><span>{activity.category.name}</span><span>{activity.points} балів</span></div><h3><Link href={`/activities/${activity.slug}`}>{activity.title}</Link></h3><p>{activity.shortDescription}</p><SkillBars skills={activity.skills} compact /><div className="activity-meta"><span>{formatDateOnly(activity.startAt)}</span><span>{activity.difficulty === 'beginner' ? 'старт' : activity.difficulty === 'intermediate' ? 'ріст' : 'виклик'}</span></div></div></article>; }
