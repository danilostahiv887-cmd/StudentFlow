...
async function uploadPendingImages(form: HTMLFormElement) {
  ...
  const inputs = Array.from(
    form.querySelectorAll<HTMLInputElement>('input[data-image-upload="true"]'),
  );
  for (const input of inputs) {
    ...
    if (!response.ok || !result.success)
      throw new Error(result.formError ?? 'Не вдалося завантажити зображення.');
    ...
    setImageField(
      root,
      'thumbnailUrl',
      typeof data.thumbnailUrl === 'string' ? data.thumbnailUrl : url,
    );
  ...
  return (
    <section className="image-picker">
      ...
      <div className={preview ? 'image-preview has-image' : 'image-preview'}>
        {preview ? (
          <img src={preview} alt={`Попередній огляд: ${altBase}`} />
        ) : (
          <span>Попередній огляд</span>
        )}
      </div>
...
export function AuthForm({ mode }: { mode: 'login' | 'register' }) {
  ...
  const submit = async (form: HTMLFormElement) => {
    ...
    const target =
      next?.startsWith('/') && !next.startsWith('//') ? next : `/${result.data?.role ?? 'student'}`;
    router.push(target);
    router.refresh();
  };
  return (
    ...
    >
      ...
      {mode === 'register' && (
        <label>
          Повне ім’я
          <AppInput name="fullName" placeholder="Ім’я та прізвище" autoComplete="name" />
          <FieldError>{fieldErrors.fullName?.[0]}</FieldError>
        </label>
      )}
      <label>
        Електронна пошта
        <AppInput name="email" type="email" placeholder="you@example.com" autoComplete="email" />
      ...
      <AppButton type="submit" disabled={pending}>
        {pending
          ? 'Зачекайте…'
          : mode === 'login'
            ? 'Увійти до StudentFlow'
            : 'Створити студентський профіль'}
      </AppButton>
    </form>
  );
}
...
export function ApplyDialog({
  ...
  const submit = async (form: HTMLFormElement) => {
    setPending(true);
    const result = await request('/api/applications', 'POST', {
      activityId,
      motivation: new FormData(form).get('motivation'),
    });
    setPending(false);
        ...
        >
          ...
          <label className="form-label">
            ...
            <AppTextarea
              name="motivation"
              rows={6}
              placeholder="Наприклад: хочу додати цей результат до портфоліо"
            />
          </label>
...
export function ReportDialog({
  ...
  const submit = async (form: HTMLFormElement) => {
    ...
    setPending(true);
    try {
      const result = await requestWithImages(form, (values) =>
        request('/api/reports', 'POST', { applicationId, ...values }),
      );
      if (!result.success) return setError(result.formError);
      setOpen(false);
            ...
            <div className="modal-section">
              <label className="form-label">
                ...
                <AppTextarea
                  name="reflection"
                  rows={5}
                  placeholder="Опишіть артефакт, рішення, виступ, файл або інший результат"
                />
              </label>
              <label className="form-label">
                Час роботи
                <AppInput name="hoursSpent" type="number" placeholder="Наприклад, 4" />
              </label>
              <label className="form-label">
                Компетентності
                <AppInput
                  name="skillsGained"
                  placeholder="Наприклад, презентація, дослідження, командна робота"
                />
              </label>
            </div>
            <ImagePicker
              label="Доказ у вигляді зображення"
              urlFieldName="evidenceUrl"
              initialUrl={initialEvidenceUrl}
              altBase={title}
              folder="/studentflow/evidence"
            />
          </div>
          <div className="dialog-actions">
            <AppButton type="button" variant="ghost" onClick={() => setOpen(false)}>
            ...
            <AppButton type="submit" disabled={pending}>
              {pending ? 'Зберігаємо…' : 'Передати доказ'}
            </AppButton>
          </div>
        </form>
      </AppModal>
...
export function ReviewDialog({
  ...
  const submit = async (form: HTMLFormElement) => {
    ...
    const result = await request('/api/reviews', 'POST', {
      ...
      id,
      status: mode,
      feedback: new FormData(form).get('feedback'),
    });
    setPending(false);
    if (!result.success) return setError(result.formError);
    setOpen(false);
      ...
      <AppModal open={open} title={title} onClose={() => setOpen(false)}>
        ...
        >
          ...
          <div className="decision-grid">
            <button
              type="button"
              className={mode === 'approved' ? 'decision active' : 'decision'}
            ...
            <button
              type="button"
              className={mode === 'needs_changes' ? 'decision active' : 'decision'}
              onClick={() => setMode('needs_changes')}
            >
              {kind === 'report' ? 'Уточнити' : 'Відкласти'}
            </button>
            <button
              type="button"
              className={mode === 'rejected' ? 'decision active' : 'decision'}
          ...
          <label className="form-label">
            ...
            <AppTextarea
              name="feedback"
              rows={4}
              placeholder="Додайте конкретний фідбек до маршруту або доказу"
            />
          </label>
          <div className="dialog-actions">
            <AppButton type="button" variant="ghost" onClick={() => setOpen(false)}>
            ...
            <AppButton type="submit" disabled={pending}>
              {pending ? 'Зберігаємо…' : 'Зберегти фідбек'}
            </AppButton>
          </div>
        </form>
      </AppModal>
...
export function CancelApplication({ applicationId }: { applicationId: string }) {
  ...
  return (
    <>
      <AppButton type="button" variant="ghost" onClick={() => setOpen(true)}>
        Прибрати
      </AppButton>
      <AppModal open={open} title="Прибрати крок" onClose={() => setOpen(false)} size="sm">
        ...
        <label className="form-label">
          ...
          <AppTextarea
            ...
            rows={3}
            placeholder="Коротко поясніть рішення"
          />
        </label>
        <div className="dialog-actions">
          <AppButton type="button" variant="ghost" onClick={() => setOpen(false)}>
          ...
          <AppButton type="button" variant="danger" onClick={() => void confirm()}>
            Прибрати з маршруту
          </AppButton>
        </div>
      </AppModal>
...
export function ToggleUserButton({ profileId, status }: { profileId: string; status: string }) {
  ...
  return (
    <>
      ...
      <ConfirmDialog
        open={open}
        title="Змінити статус користувача"
        message="Користувач одразу втратить або отримає доступ до робочого простору."
...
export function StudentBadgeToggle({
  studentId,
  badgeId,
  studentBadgeId,
}: {
  ...
  const grant = async () => {
    setPending(true);
    const result = await request('/api/admin', 'POST', {
      action: 'studentBadge',
      studentId,
      badgeId,
    });
  ...
  const revoke = async () => {
    ...
    setPending(true);
    const result = await request('/api/admin', 'DELETE', {
      entity: 'studentBadge',
      id: studentBadgeId,
    });
    setPending(false);
  ...
  return (
    <>
      {studentBadgeId ? (
        <AppButton type="button" variant="danger" disabled={pending} onClick={() => setOpen(true)}>
          Забрати
        </AppButton>
      ) : (
        <AppButton
          ...
          onClick={() => void grant()}
        >
          {pending ? 'Видаємо…' : 'Видати'}
        </AppButton>
      )}
      <ConfirmDialog
        open={open}
        title="Забрати відзнаку"
        message="Відзнака зникне з портфоліо студента. Її можна буде видати повторно вручну."
        confirmText={pending ? 'Зачекайте…' : 'Забрати'}
        danger
        onCancel={() => setOpen(false)}
...
export function ProfileForm({
  ...
  return (
    ...
    >
      ...
      <label className="form-label">
        Повне ім’я
        <AppInput name="fullName" defaultValue={profile.fullName} />
      </label>
      <label className="form-label">
        Телефон
        <AppInput name="phone" defaultValue={profile.phone} placeholder="+380 …" />
      </label>
      {profile.role === 'student' && (
        <div className="dashboard-grid">
          <label className="form-label">
            Група
            <AppSelect name="groupId" defaultValue={profile.groupId}>
              {groups.map((item) => (
          ...
          <label className="form-label">
            Спеціальність
            <AppSelect name="specialityId" defaultValue={profile.specialityId}>
      ...
      <label className="form-label">
        ...
        <AppTextarea
          ...
          rows={4}
          placeholder="Коротко про свої цілі"
        />
      </label>
      <AppButton type="submit" disabled={pending}>
...
export function AdminCreateDialog({
  ...
  kind: 'teacher' | 'activity' | 'reference';
  ...
  const submit = async (form: HTMLFormElement) => {
    ...
    try {
      const result = await requestWithImages(form, (values) =>
        request('/api/admin', 'POST', { action: kind, ...values }),
      );
      if (!result.success) return setError(result.formError);
      setOpen(false);
      push('Дані збережено.');
          ...
          {kind === 'activity' && (
            <div className="modal-grid modal-grid-2">
              <div className="modal-section">
                ...
                <label className="form-label">
                  Короткий опис
                  <AppTextarea name="shortDescription" rows={3} />
                </label>
                ...
                <label className="form-label">
                  Локація
                  <AppInput name="location" />
                </label>
                <div className="modal-grid modal-grid-2 compact">
                  <label className="form-label">
                    Старт
                    <AppInput name="startAt" type="datetime-local" />
                  </label>
                  <label className="form-label">
                    Фініш
                    <AppInput name="endAt" type="datetime-local" />
                  </label>
                  <label className="form-label">
                    Ліміт учасників
                    <AppInput name="maxParticipants" type="number" />
                  </label>
                  <label className="form-label">
                    Цінність
                    <AppInput name="points" type="number" />
                  </label>
                </div>
              </div>
              <ImagePicker
                label="Обкладинка можливості"
                altBase="Нова можливість"
                folder="/studentflow/activities"
              />
            </div>
          )}
          {kind === 'reference' && (
  ...
  if (field.type === 'image') {
    return (
      <ImagePicker
        label={field.label}
        help={field.help}
        initialUrl={String(field.value ?? '')}
        altBase={field.alt ?? title}
      />
    );
  }
  return (
    <label className="form-label">
      {field.label}
      {field.type === 'textarea' ? (
        <AppTextarea name={field.name} defaultValue={String(field.value ?? '')} rows={4} />
      ) : field.type === 'select' ? (
        <AppSelect name={field.name} defaultValue={String(field.value ?? '')}>
...
export function AdminCrudActions({
  ...
  const save = async (form: HTMLFormElement) => {
    ...
    try {
      const result = await requestWithImages(form, (values) =>
        request('/api/admin', 'PUT', { entity, id, values }),
      );
      if (!result.success) return setError(result.formError);
      setEditOpen(false);
  ...
  return (
    <div className="data-row-actions">
      <AppButton type="button" variant="secondary" onClick={() => setEditOpen(true)}>
        Редагувати
      </AppButton>
      ...
      <AppModal
        ...
        size={fields.length > 4 || fields.some((field) => field.type === 'image') ? 'lg' : 'md'}
      >
        <form
          className="modal-form"
          noValidate
        ...
        >
          ...
          <div className="modal-grid modal-grid-2">
            {fields.map((field) => (
              <CrudFieldControl field={field} title={title} key={field.name} />
            ))}
          </div>
          <div className="dialog-actions">
            <AppButton type="button" variant="ghost" onClick={() => setEditOpen(false)}>
              Скасувати
            </AppButton>
            <AppButton type="submit" disabled={pending}>
      ...
      <ConfirmDialog
        ...
        message="Запис буде видалено разом із пов’язаними маршрутами, доказами, відзнаками або медіа, якщо вони залежать від цього запису."
